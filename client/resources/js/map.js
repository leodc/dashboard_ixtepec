...
function buildPopup(layer){
    // var properties = layer.feature.properties;
    // var time = moment(properties.captured_at);
    // var content = "<img width='" + popupSize + "px' src='https://d1cuyjsrcm0gby.cloudfront.net/" + properties.key + "/thumb-" + popupSize + ".jpg' />";
    // content += "<br>Fecha de la imagen: " + time.format("MMMM Do YYYY, h:mm:ss a");
    // content += "<br><button id='" + properties.key + "' onclick='buildMplViewer(this.id)'>Iniciar visualizador</button>"
    // return content;
